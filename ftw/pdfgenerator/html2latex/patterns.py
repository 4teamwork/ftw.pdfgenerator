...
# generally remove empty no-singleton-tags
    (MODE_REGEXP,             r'<(?P<tag>[\w]{0,})>[\s]{0,}</(?P=tag)?>',    r'',         (interfaces.HTML2LATEX_REPEAT_MODIFIER)),
...
# headings
    ...
    (MODE_REGEXP,             r'<h3.*?>(.*?)</h3>',      r'\\subsubsection{\g<1>}\n'),
...
# font style and text decoration
    ...
    (MODE_REGEXP,             r'([^ ])<(b|strong|em|u|i)> ',
                                                         r'\g<1> <\g<2>>'),
    (MODE_REGEXP,             r'<b>(.*?)</b>',           r'{\\bf \g<1>}' + interfaces.HTML2LATEX_PREVENT_CHARACTER),
    (MODE_REGEXP,             r'<b .*?>(.*?)</b>',       r'{\\bf \g<1>}' + interfaces.HTML2LATEX_PREVENT_CHARACTER),
    ...
    (MODE_REGEXP,             r'<sup.*?>(.*?)</sup>',    r'\\textsuperscript{\g<1>}'),
# quotes
    (MODE_REGEXP,             r'\&quot;',                 '"'),
    (MODE_REGEXP,             r'"(\w)',                   '"`\g<1>'),
    (MODE_REGEXP,             r'(\S)"',                   '\g<1>"\''),
...
# special characters
    ...
    (MODE_REPLACE,             '^',                       '\\^{}'),
    (MODE_REPLACE,             '~',                       '\\~{}'),
# Various abbreviation replacements... TODO: make regexp patterns or subconverter
...
# date replacements: we use nonbreakable spaces TODO: make regexp patterns or subconverter
   ] + [
   (MODE_REGEXP,              r'(\d)\.\W{0,1}(' + month + ')',
                                                          r'\1.~\2')
       for month in ['Jan', 'Feb', 'Mar', 'Mär', 'Apr', 'Mai', 'May', 'Jun', 'Jul',
...
# NOT SUPPORTET!!
    ...
    (MODE_REPLACE,             '►',                       ''),
# misc
    (MODE_REGEXP,             r'([a-zA-Z])-',            r'\1"='),            # - at the end of a word should not disable hyphenation
#    (MODE_REGEXP,             r'([a-zA-Z].)*',           r'U\1U'),
                    # abbreviation should contain short whitespace (\,)
# cleanup
    (MODE_REGEXP,             r'<([^>]*?)>',             r''),                # strip html tags
    (MODE_REGEXP,             r'\\\\\s*(\n\\sub)',       r'\g<1>'),           # remove all line endings (\\) before \sub*
    (MODE_REGEXP,             r'(\n|^)[ ]{0,}\\\\',      r'\g<1>'),           # remove all line endings (\\) which finish a empty line
    (MODE_REGEXP,             r'}[\t ]{0,}\\\\',         r'}'),               # remove all line endings (\\) after a } (latex command)
# special characters
    (MODE_REPLACE,             '&lt;',                    '<'),
    (MODE_REPLACE,             '&gt;',                    '>'),
    (MODE_REPLACE,             '&',                       '\\&'),
